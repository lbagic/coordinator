...
export function protocolOf(name, opts = {}) {
  ...
  switch (opts.kind) {
    ...
    case 'implement':
      return [
        `Protocol (implement): deliver the way the Fences say this repo works: a worktree and a PR babysat to CI green with every review-bot thread answered, or commits on main with the issue number in the subject; the Fences name the base and the route. Do your own recon; one concern per PR or commit; checks scoped to the diff, run by you, their evidence quoted. You never merge: that is the user's. You push your own work and nothing else, by the route the Fences name: your branch and its PR, or your commits on main. Commits and the PR are authored as the user: the repo git identity, no Co-Authored-By, no session or generated-with trailer, no AI attribution in messages or bodies.`,
